...
<div id="header">
	<h2>MongoDB Browser Shell (Beta)</h2>
</div>

<div id="terminal">
	<p class="response">MongoDB browser shell beta</p>
	<br />
	<p>Note: this shell supports only a subset of the real shell's features.
	Once you've mastered the basics, <a href="http://mongodb.org/downloads">download MongoDB</a> to
	continue experimenting.</p><br/>
...
ReadLine.prototype = {
  ...
  initialize: function() {
    ...
    $.ajax({url: "api/me", type: 'GET', async: false, success: function(result){
      username = result.uid.toString();
    }});

	socket.emit('mongodb.removeDb', username, function (err, data) {
	});

	socket.emit('mongodb.initial', username, function (err, data) {
	});
  ...
  processInput: function(value) {
    ...
    if(typeof response.result == 'object'){
    	response.result.forEach(function(value) {
	    	stringRes = stringRes + value  + '<br>';
		})
		response.result = stringRes;
    }
...
MongoHandler.prototype = {
  ...
  _help: function() {
      return PTAG('HELP') + 
             ...
             // PTAG('db.collection.help()                            help on collection methods') +
             PTAG('db.collection.find()                             list objects in collection collection') +
             PTAG('db.collection.save({a: 1})                  save a document to collection collection') +
             PTAG('db.collection.update({a: 1}, {a: 2})   update document where a == 1') +
             PTAG('db.collection.find({a: 1})                   list objects in collection where a == 1') + 
             PTAG('db.collection.remove({a: 1})             remove object in collection where a == 1');
  },
  ...
  _showCollections: function() {
  	var uid, collections, results;
  	$.ajax({url: "api/me", type: 'GET', async: false, success: function(result){
		uid = result.uid.toString();
    }});

	$.ajax({url: "api/terminal/showCol?uid=" + uid, type: 'GET', async: false, success: function(result){
      results = result;
    }});
